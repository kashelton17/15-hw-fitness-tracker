...
router.get("/api/workouts/range", async (req, res) => {
    try {
        await Workout.aggregate([{ "$addFields": {"totalDuration": {$sum: "$exercises.duration" },"totalWeight": {$sum: "$exercises.weight" }, "totalReps": {$sum: "$exercises.reps" },"totalSets": {$sum: "$exercises.sets" }}}]).limit(7).then((dbWorkout) => {
            res.json(dbWorkout);
